...
main() {
  log_info_hook "main" "pre-commit-prettier" "Started"
  if [ -f ".pre-commit-config.yaml" ]; then
    current_version=$(get_current_version)
    latest_version=$(get_latest_release "prettier/prettier" | sed -E 's/^v//')
    if [ -n "${current_version}" ] && [ -n "${latest_version}" ] && [ "${current_version}" != "${latest_version}" ]; then
      log_info_hook "main" "pre-commit-prettier" "Updating prettier from version ${current_version} to ${latest_version}"
      ...
      log_info_hook "main" "pre-commit-prettier" "prettier is already at the latest ${latest_version} version"
    fi
  else
    log_info ".pre-commit-config.yaml file is not found"
  fi
  log_info_hook "main" "pre-commit-prettier" "Completed"
}
